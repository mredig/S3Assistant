...
public struct S3FileMetadata: Codable, CustomStringConvertible {
	...
	init(from xmlNode: XMLNode, delimiter: String) throws {
		...
		else { throw WasabiFileMetadataError.invalidXMLNode(reason: "Invalid Key Node", node: xmlNode) }
		self.key = keyValue
		...
		else { throw WasabiFileMetadataError.invalidXMLNode(reason: "Invalid LastModified Node", node: xmlNode) }
		...
		else { throw WasabiFileMetadataError.invalidXMLNode(reason: "Invalid ETag Node", node: xmlNode) }
		self.eTag = eTagValue
		...
		else { throw WasabiFileMetadataError.invalidXMLNode(reason: "Invalid Size Node", node: xmlNode) }
		self.size = sizeValue
		...
		else { throw WasabiFileMetadataError.invalidXMLNode(reason: "Invalid StorageClass Node", node: xmlNode) }
		self.storageClass = storageClassValue
	}

	enum WasabiFileMetadataError: Error {
